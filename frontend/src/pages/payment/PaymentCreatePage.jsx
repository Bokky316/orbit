...
// 인보이스 담당자인지 확인 (나중에 필터링된 송장 목록 길이로 판단)
const isInvoiceApprover = () => {
...
// 미결제 송장 목록 조회 - 담당자별 필터링 수정
const fetchUnpaidInvoices = async () => {
  try {
    setLoading(true);

    // API 엔드포인트와 파라미터 설정
    let endpoint = `${API_URL}invoices/list`;
    const params = new URLSearchParams({
      status: 'APPROVED',  // 승인된 상태의 송장만 조회
      page: 0,
      size: 100,
      sortBy: 'issueDate',
      sortDir: 'desc'
    ...
    // 관리자는 모든 송장을 볼 수 있음
    if (isAdmin()) {
      endpoint = `${API_URL}invoices/list?${params.toString()}`;
    }
    // 관리자가 아닌 경우(재무회계팀 포함)는 담당자별 필터링
    else {
      // 담당자 본인의 송장만 조회하는 엔드포인트 호출
      endpoint = `${API_URL}invoices/list/approver/${currentUser.id}?${params.toString()}`;
    }
    ...
    console.log('송장 개수:', data.invoices ? data.invoices.length : 0);
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
            ...
            {unpaidInvoices.length === 0 ? (
              <Alert severity="warning" sx={{ mt: 2 }}>
                {isAdmin()
                  ? "결제 가능한 송장이 없습니다. 먼저 송장을 승인해주세요."
                  : "담당자로 배정된 결제 가능한 송장이 없습니다."
                }
              </Alert>
            ) : (
              <Autocomplete
                  ...
                  <li {...props}>
                    <Box sx={{ width: '100%' }}>
                      ...
                      <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                        ...
                        {option.approverName && <span> | 담당자: {option.approverName}</span>}
                      </Typography>
                    </Box>
                  </li>
                )}
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  ...
                  <InfoRow>
                    ...
                    <Typography className="value">{selectedInvoice.approverName || '지정되지 않음'}</Typography>
                  </InfoRow>
                </Grid>
                <Grid item xs={12} md={6}>
                  <InfoRow>
