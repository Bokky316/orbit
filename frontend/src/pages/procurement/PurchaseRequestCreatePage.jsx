...
function PurchaseRequestCreatePage() {
  ...
  // Redux 상태 가져오기
  const { items: availableItems, categories } = useSelector(
  ...
  useEffect(() => {
    ...
    const fetchAllProjects = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `프로젝트 목록을 가져오는데 실패했습니다: ${response.status}`
          );
        }
        ...
        alert(
          `프로젝트 목록을 가져오는 중 오류가 발생했습니다: ${error.message}`
        );
      }
    };
    ...
    const fetchDepartments = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `부서 목록을 가져오는데 실패했습니다: ${response.status}`
          );
        }
        ...
        console.error(
          `부서 목록을 가져오는 중 오류가 발생했습니다: ${error.message}`
        );
      }
    };
    ...
    const fetchAllMembers = async () => {
      try {
        ...
        console.error(
          `사용자 목록을 가져오는 중 오류가 발생했습니다: ${error.message}`
        );
      }
    };
  ...
  useEffect(() => {
    if (selectedManager && selectedManager.contactNumber) {
      setManagerPhoneNumber(
        selectedManager.contactNumber.replace(/[^0-9]/g, "")
      );
    } else {
      setManagerPhoneNumber("");
    }
      ...
      requestData.items = items.map((item) => {
        ...
        return {
          ...
          deliveryRequestDate: item.deliveryRequestDate
            ? item.deliveryRequestDate.format("YYYY-MM-DD")
            : null,
        ...
        if (attachments.length > 0) {
          ...
          try {
            ...
            const fileResponse = await fetch(
              `${API_URL}purchase-requests/${createdRequest.id}/attachments`,
              {
                method: "POST",
          ...
          <>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) =>
                  setContractAmount(e.target.value.replace(/[^0-9]/g, ""))
                }
                InputProps={{
                  startAdornment: (
              ...
              <Grid
                ...
                <Grid item xs={1}>
                  <TextField
                    ...
                    value={item.quantity}
                    onChange={(e) =>
                      handleNumericItemChange(index, "quantity")(e)
                    }
                    required
                  />
                </Grid>
                ...
                <Grid item xs={2}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleNumericItemChange(index, "unitPrice")(e)
                    }
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">₩</InputAdornment>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={6}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="사업 부서 *" required />
                )}
              />
            </Grid>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    required
                    helperText={
                      !selectedDepartment ? "먼저 사업 부서를 선택해주세요" : ""
                    }
                  />
                )}
              />
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) =>
                  setBusinessBudget(e.target.value.replace(/[^0-9]/g, ""))
                }
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">₩</InputAdornment>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) =>
                  setManagerPhoneNumber(e.target.value.replace(/[^0-9]/g, ""))
                }
                InputProps={{
                  startAdornment: (
