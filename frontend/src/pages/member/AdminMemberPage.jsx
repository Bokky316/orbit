...
export default function AdminMemberPage() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `${API_URL}members?${queryParams.toString()}`,
        {
          method: "GET"
        }
      );
      ...
      if (response.ok) {
        ...
        showAlert(
          `회원 목록을 불러오는데 실패했습니다. (${response.status})`,
          "error"
        );
        setMembers([]);
        setTotalItems(0);
  ...
  const toggleMemberStatus = async (id) => {
    try {
      const response = await fetchWithAuth(
        `${API_URL}members/${id}/toggle-status`,
        {
          method: "POST"
        }
      );
  ...
  const updateMemberRole = async () => {
    ...
    try {
      const response = await fetchWithAuth(
        `${API_URL}members/${selectedMember.id}/role`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        showAlert(
          errorData.message || "회원 역할 변경에 실패했습니다.",
          "error"
        );
      }
    } catch (error) {
        ...
        try {
          ...
          try {
            ...
            if (cleanedText.trim().startsWith("{")) {
              ...
              const contactNumberMatch = /"contactNumber":"([^"]+)"/.exec(
                cleanedText
              );
              const postalCodeMatch = /"postalCode":"([^"]+)"/.exec(
                cleanedText
              );
              const roadAddressMatch = /"roadAddress":"([^"]+)"/.exec(
                cleanedText
              );
              const detailAddressMatch = /"detailAddress":"([^"]+)"/.exec(
                cleanedText
              );
  ...
  const updateMemberDetail = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `${API_URL}members/${memberDetail.id}`,
        {
          method: "PUT",
      ...
      if (response.ok) {
        try {
          ...
          showAlert(
            "회원 정보가 수정되었으나 서버 응답을 처리할 수 없습니다.",
            "warning"
          );
          setOpenDetailDialog(false);
          fetchMembers(); // 목록 새로고침
        ...
        try {
          ...
          if (errorText && errorText.includes("message")) {
            try {
              ...
              showAlert(
                errorJson.message || "회원 정보 수정에 실패했습니다.",
                "error"
              );
            } catch (e) {
            ...
            showAlert(
              "회원 정보 수정에 실패했습니다. 서버 오류가 발생했습니다.",
              "error"
            );
          }
        } catch (error) {
  ...
  return (
    <PageContainer>
      <PageTitle variant="h4">사용자 목록</PageTitle>
      ...
      {/* 알림 메시지 */}
      {alert.show && (
        <AlertStyled severity={alert.severity}>{alert.message}</AlertStyled>
      )}
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal" disabled>
                  ...
                  <Select
                    value={
                      memberDetail.role ? memberDetail.role.toUpperCase() : ""
                    }
                    label="역할">
                    <MenuItem value="BUYER">구매자 (BUYER)</MenuItem>
                    <MenuItem value="SUPPLIER">공급자 (SUPPLIER)</MenuItem>
