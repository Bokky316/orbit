...
const InvoiceDetailPage = () => {
  ...
  const canMarkAsPaid = () => {
    ...
    // 대기 상태 또는 연체 상태의 송장만 결제 완료 처리 가능
    if (invoice.status !== 'WAITING' && invoice.status !== 'OVERDUE') return false;
  ...
  const handleMarkAsPaid = async () => {
    try {
      setLoading(true);
      const response = await fetchWithAuth(`${API_URL}invoices/${id}/payment-complete`, {
        method: 'PUT'
      });
      ...
      setSuccessMessage('결제 완료 처리가 성공적으로 완료되었습니다.');
      setShowSuccess(true);
    } catch (error) {
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <InfoRow>
                  ...
                  <Typography className="value">{invoice.contractNumber || '-'}</Typography>
                </InfoRow>
                <InfoRow>
                  <Typography className="label">거래 번호:</Typography>
                  <Typography className="value">{invoice.transactionNumber || '-'}</Typography>
                </InfoRow>
                <InfoRow>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <InfoRow>
                  ...
                  <Typography className="value">{invoice.userName || '-'}</Typography>
                </InfoRow>
                <InfoRow>
                  <Typography className="label">공급자명:</Typography>
                  <Typography className="value">{invoice.supplierName || '-'}</Typography>
                </InfoRow>
                <InfoRow>
                  <Typography className="label">담당자:</Typography>
                  <Typography className="value">{invoice.supplierContactPerson || '-'}</Typography>
                </InfoRow>
              </Grid>
              <Grid item xs={12} md={6}>
                <InfoRow>
                  <Typography className="label">이메일:</Typography>
                  <Typography className="value">{invoice.supplierEmail || '-'}</Typography>
                </InfoRow>
                <InfoRow>
                  <Typography className="label">전화번호:</Typography>
                  <Typography className="value">{invoice.supplierPhone || '-'}</Typography>
                </InfoRow>
                <InfoRow>
                  <Typography className="label">주소:</Typography>
                  <Typography className="value">
                      {invoice.supplierAddress && invoice.supplierAddress.trim() !== ''
