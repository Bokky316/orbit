...
function DeliveryDetailPage() {
  ...
  const findCompanyName = () => {
    ...
    if (isSupplier()) {
      ...
      const company = user.companyName || user.company || user.supplierName;
      ...
      <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            {delivery && (
              <>
                입고번호 <strong>{delivery.deliveryNumber}</strong>의 정보를
                삭제하시겠습니까?
                <br />이 작업은 되돌릴 수 없습니다.
              </>
            )}
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
            ...
            <Box sx={{ display: "flex", mt: 2, flexWrap: "wrap" }}>
              <Box sx={{ display: "flex", mr: 4, alignItems: "center" }}>
                <Typography
                  sx={{ fontWeight: 500, color: "text.secondary", mr: 1 }}>
                  담당자:
                </Typography>
                ...
                <InfoRow>
                  ...
                  <Typography className="value">
                    {delivery.deliveryDate
                      ? moment(delivery.deliveryDate).format("YYYY-MM-DD")
                      : "-"}
                  </Typography>
                </InfoRow>
              </Grid>
