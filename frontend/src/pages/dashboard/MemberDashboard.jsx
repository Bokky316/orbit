...
const MemberDashboard = () => {
  ...
  const getFilteredRequests = () => {
    ...
    const statusFiltered = filterRequestsByStatus(
    ...
    if (selectedStatus === "IN_PROGRESS" && subStatusTab !== "ALL") {
      subStatusFiltered = statusFiltered.filter(
        (req) => req.status === subStatusTab
      );
    }
    ...
    return subStatusFiltered.filter(
      (request) =>
        (request.requestName &&
          request.requestName
            .toLowerCase()
  ...
  const getListTitle = () => {
    ...
    if (selectedStatus === "IN_PROGRESS" && subStatusTab !== "ALL") {
      const subStatusLabel =
        inProgressSubStatuses.find((s) => s.code === subStatusTab)?.label || "";
          ...
          <Card
            ...
            <CardContent>
              ...
              <Typography
                variant="h6"
                gutterBottom
                sx={{ fontWeight: 600, color: "#2c3345" }}>
                구매요청 현황
              </Typography>
        ...
        <Grid item xs={12} md={4}>
          <Card variant="outlined">
            <CardContent>
              ...
              {/* 구매요청 목록 */}
              <RecentRequestsList requests={getFilteredRequests()} />
            </CardContent>
          </Card>
        </Grid>
